...
class RoleController extends Controller
{
        ...
        return $this->createFormBuilder([])
            ->setAction(
                ...
                array(
                    ...
                    'placeholder' => $this->get('translator')
                        ->trans('getAddable.selected.placeholder', [], $this->transDomain),
                    'attr' => array(
                        'class' => 'form-control',
                    ),
                )
    ...
    public function addUserAction(Request $request, Project $project)
    {
        ...
        if ($form->isValid()) {
            ...
            $user = $this->getDoctrine()
                ->getRepository("Cscfa\Bundle\SecurityBundle\Entity\User")
                ->findOneById($dataArray['selected']);
    ...
    public function updateUserRoleAction(Request $request)
    {
        ...
        $owner = $this->getDoctrine()
            ->getRepository("Cscfa\Bundle\CSManager\ProjectBundle\Entity\ProjectOwner")
            ->find($owner);
        if (!$owner) {
