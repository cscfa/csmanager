...
class PhoneValidator extends ConstraintValidator
{
     ...
     *
     * @param string $phoneString - the phone number
     * @param int    $slElement   - the phone start lenght
     *
     * @return number
     */
    public function getCountryCodeLength($phoneString, $slElement)
    {
        $countryCodes = array(
            '1',    '7',    '20',   '27',   '30',   '31',
            '32',   '33',   '34',   '36',   '39',   '40',
        ...
        $phoneString = substr($phoneString, $slElement);
     ...
     *
     * @param string $phoneString - the phone number
     * @param int    $cclElement  - the country code length
     * @param int    $slElement   - the phone start lenght
     *
     * @return bool
     */
    public function isAnalysable($phoneString, $cclElement, $slElement)
    {
        $phoneString = substr($phoneString, ($slElement + $cclElement));
     ...
     * @return bool
     */
    public function maxOverflowValid($phoneString, $slElement)
    {
        return (strlen($phoneString) - $slElement) <= 15;
    }
     ...
     * @return bool
     */
    public function isNumber($phoneString, $slElement)
    {
        return is_numeric(substr($phoneString, $slElement));
    }
    ...
    {
        $slElement = $this->getStartLength($value);

        if (!$this->isNumber($value, $slElement) || !$this->maxOverflowValid($value, $slElement) || $slElement == 0) {
            $this->context->buildViolation($constraint->message)->addViolation();
        }

        if ($this->isInternational($value)) {
            $ccl = $this->getCountryCodeLength($value, $slElement);

            if ($ccl == 0) {
                $this->context->buildViolation($constraint->message)->addViolation();
            } elseif (!$this->isAnalysable($value, $ccl, $slElement)) {
                $this->context->buildViolation($constraint->message)->addViolation();
            }
        }
    }
