...
class AddCollectionEntityChainTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->addCollection = new AddCollectionEntityChain();
        ...
        $this->addCollection->setEntityRepository($this->entityRepository)
    ...
    public function testUnsupportedProcess()
    {
        ...
        $this->addCollection->setNext($this->next);
        $this->assertSame(
            $this->next,
            $this->addCollection->process(
                self::UNSUPPORTED_PROPERTY_NAME,
    ...
    public function testProcessWithTrueObject()
    {
        ...
        $this->assertNull(
            $data->testProperty,
            "The AddCollectionEntityChain mustn't inject data into property if no one setter exist and data" +
            'to inject not match the needles'
        );
    ...
    {
        $idGenerated = openssl_random_pseudo_bytes(10);
        $entity = clone $this->currentEntity;
        $repository = clone $this->entityRepository;

        $repository->expects($this->exactly(3))
            ->method('find')
            ->with($this->equalTo($idGenerated))
            ->will($this->returnValue($entity));

        $this->addCollection->setEntityRepository($repository);
        ...
        $this->addCollection->process(
            self::SUPPORTED_PROPERTY_NAME,
            $data,
            array('data' => $idGenerated)
        );

        $this->assertSame(
            array($entity),
            $data[self::SUPPORTED_PROPERTY_NAME],
            'The AddCollectionEntityChain must inject the repository finded object by method if the data' +
            'injected is an id and entity exist'
        );
        ...
        $this->assertFalse(
            array_key_exists(self::SUPPORTED_PROPERTY_NAME, $data),
            "The AddCollectionEntityChain mustn't inject the repository finded object by method if" +
            'the data injected is an id and entity not exist'
        );
    }
}
