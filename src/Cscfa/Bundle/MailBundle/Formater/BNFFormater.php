...
class BNFFormater
{
     ...
     *
     * @param int $key the element index to remove
     */
     ...
     * null.
     *
     * @param int $key The element index to get.
     *
     ...
     *
     * @param int $key The index to check
     *
     * @return bool
     */
     ...
     * @param int        $key     The key where store
     *
     * @return int The key where the element is stored
     */
     ...
     * have name.
     *
     * @param bool $withoutComment  Automaticaly skip the comment
     * @param bool $onlySignificant Automaticaly skip the elements without
     *                              significants
     *
     * @return string
     */
    public function dump($withoutComment = false, $onlySignificant = false)
    {
        ...
        foreach ($this->elements as $key => $element) {
            if ($element instanceof BNFElement) {
                ...
                $significant = !$element->hasSignificant() && $onlySignificant;
                $withoutComment = !$element->hasSignificant() && $withoutComment;
                if ($significant || $withoutComment) {
                    if ($key === $keySet[0]) {
                        array_shift($keySet);
                    }
                    continue;
                ...
                if ($element->getName() !== '') {
                    ...
                    $litteral = $element->getLitteral() !== '';
                    $comment = $element->getComment() !== '';
                    if ($litteral || ($comment && !$withoutComment)) {
                        $result .= ' ';
                    }
                }
     ...
     *
     * @example "name <litteral(comment)>(other comment)" will return
     *          array("comment, other comment", "name <litteral>")
     *
     * @return array
     */
