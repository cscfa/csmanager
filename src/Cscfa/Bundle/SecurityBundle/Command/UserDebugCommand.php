...
class UserDebugCommand extends ContainerAwareCommand
{
     ...
     * behind "app/console cs:debug:user".
     *
     * @see    \Symfony\Component\Console\Command\Command::configure()
     */
    protected function configure()
    {
        $this->setName('cs:debug:user')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        list($rows, $error) = $commandFacade->debugMulti(
            ...
            array(
                array(
                    'target' => 'getUsernameCanonical',
                    'test' => new CanonicalTest('getUsername'),
                ),
                array(
                    'target' => 'getEmailCanonical',
                    'test' => new CanonicalTest('getEmail'),
                ),
                array(
                    'target' => 'getLastLogin',
                    'test' => new DateTimeTest(
                        DateTimeTest::BEFORE_NOW +
                        DateTimeTest::CURRENT +
                        DateTimeTest::ALLOW_NULL
                    ),
                ),
                array(
                    'target' => 'getPasswordRequestedAt',
                    'test' => new DateTimeTest(
                        DateTimeTest::BEFORE_NOW +
                        DateTimeTest::CURRENT +
                        DateTimeTest::ALLOW_NULL
                ...
                array(
                    'target' => 'getUpdatedAt',
                    'test' => new UpdateAtTest(),
                ),
                array(
                    'target' => 'getCreatedAt',
                    'test' => new DateTimeTest(DateTimeTest::BEFORE_NOW),
                ),
                array(
                    'target' => 'getCreatedBy',
                    'test' => new UserInstanceTest(),
                ),
                array(
                    'target' => 'getUpdatedBy',
                    'test' => new UpdatorTest(),
                ),
            ),
